...
import User from '../models/userSignUpModel.js';
import { sendScholarshipAwardEmail } from '../utils/backEndScholarshipAwardEmail.js';
...
const sendAwardEmailsForBackEnd = async (request, response) => {
  try {
    const course = 'Back-End Web Development';
    const now = new Date();
    const fiveDaysAgo = new Date(now.getTime() - 5 * 24 * 60 * 60 * 1000);
...
export default sendAwardEmailsForBackEnd;